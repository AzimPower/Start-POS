...
const CustomTooltip = ({ active, payload, label, color }: any) => {
    ...
    const formatted = typeof value === 'number'
        ...
        : value;
    return (<div className="bg-white border border-gray-100 rounded-xl shadow-lg px-4 py-2.5 text-sm">
...
export default function SalesChart({ data, xKey, yKey, color = '#4ade80', chartType = 'line' }: SalesChartProps) {
    ...
    return (<ResponsiveContainer width="100%" height={300}>
      {chartType === 'bar' ? (<BarChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
          <defs>
          ...
          <YAxis tick={{ fontSize: 11, fill: '#9ca3af' }} axisLine={false} tickLine={false} tickFormatter={(v) => {
                const n = Number(v);
        ...
        </BarChart>) : (<AreaChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
          <defs>
            <linearGradient id={gradientId} x1="0" y1="0" x2="0" y2="1">
          ...
          <Area type="monotone" dataKey={yKey} stroke={color} strokeWidth={2.5} fill={`url(#${gradientId})`} isAnimationActive={false} connectNulls={true} dot={(props) => {
                const key = (props as any).index ?? `${props.cx}-${props.cy}`;
                if (props.cx == null || props.cy == null)
                    return null;
                return (<circle key={key} cx={props.cx} cy={props.cy} r={3.5} fill={color} stroke="#ffffff" strokeWidth={2} style={{ pointerEvents: 'none' }}/>);
            }}/>
        </AreaChart>)}
    </ResponsiveContainer>);
}
