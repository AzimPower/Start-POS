...
const Dialog = (props: React.ComponentProps<typeof DialogPrimitive.Root>) => {
  ...
  React.useEffect(() => {
    const entry = { forceClose: () => {
      ...
      setOpenState(false);
      try { if (typeof props.onOpenChange === 'function') props.onOpenChange(false); } catch (e) {}
    } };
    try {
      if (typeof window !== 'undefined') (window as any)[REGISTRY_KEY].add(entry);
    } catch (e) {}
    return () => {
      try { if (typeof window !== 'undefined') (window as any)[REGISTRY_KEY].delete(entry); } catch (e) {}
    };
  }, []);
  ...
  return (
    <DialogPrimitive.Overlay
      ...
      className={cn(
        ...
        "radix-dialog-overlay fixed inset-0 z-50 bg-black/80 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0",
        className,
      )}
...
>(({ className, children, ...props }, ref) => {
  ...
  const handlePointerDownOutside: any = (event: any) => {
    ...
    if (typeof (props as any).onPointerDownOutside === 'function') (props as any).onPointerDownOutside(event);
  };
  ...
  const handleEscapeKeyDown: any = (event: any) => {
    ...
    if (typeof (props as any).onEscapeKeyDown === 'function') (props as any).onEscapeKeyDown(event);
  };
    ...
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          ...
          "fixed z-50 grid w-auto min-w-[90vw] sm:min-w-[400px] max-w-2xl gap-4 border bg-background shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] sm:rounded-lg p-4 sm:p-6 left-4 right-4 top-1/2 sm:left-1/2 sm:translate-x-[-50%] translate-y-[-50%] overflow-hidden",
          className,
        )}
