...
export default function ShiftReceiptDetails({ selectedShift, cashiers }: {
    ...
    useEffect(() => {
        ...
        async function fetchStoreName() {
            if (!selectedShift?.storeId)
                return setStoreName('');
            try {
                const db = await import('@/lib/db').then(m => m.getDB());
                const store = await db.get('stores', selectedShift.storeId);
                if (isMounted)
                    setStoreName(store?.name || selectedShift.storeId || '-');
            }
            catch {
    ...
    const [refundsSummary, setRefundsSummary] = useState<{
        cash: number;
        mobile_money: number;
    } | null>(null);
  ...
  {refundsSummary && (refundsSummary.cash > 0 || refundsSummary.mobile_money > 0) && (<div style={{ marginTop: 8, padding: '4px 0', borderTop: '1px dashed #ccc', borderBottom: '1px dashed #ccc' }}>
      <div style={{ fontWeight: 'bold', marginBottom: 4, textAlign: 'center' }}>Remboursements</div>
      {refundsSummary.cash > 0 && (<div style={{ display: 'flex', justifyContent: 'space-between' }}><span>  Espèces :</span> <b>{formatMoney(refundsSummary.cash)} FCFA</b></div>)}
      {refundsSummary.mobile_money > 0 && (<div style={{ display: 'flex', justifyContent: 'space-between' }}><span>  Mobile Money :</span> <b>{formatMoney(refundsSummary.mobile_money)} FCFA</b></div>)}
