...
export default function CustomerReceipts() {
    ...
    const updatePendingSyncCount = async (db: any) => {
        try {
            ...
            const salesPendingOps = syncQueue.filter(op => op.table === 'sales' && op.storeId === user?.storeId);
            ...
            if (isOnline) {
                try {
                    ...
                    if (response.ok) {
                        ...
                        processSales(mergedBackendSales, db);
                    }
                }
                catch (error) {
    ...
    const processSales = async (allSales: any[], db: any) => {
        ...
        // Filtrer par client
        let salesData = normalizedSales.filter(sale => sale.customerId && String(sale.customerId) === String(customerId));
          ...
          <CardContent className="p-0 sm:p-6">
            <div className="overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {filteredSales.length === 0 ? (<TableRow>
                ...
                const store = stores.find(s => s.id === sale.storeId);
                const receiptNumber = `REC${sale.id.slice(-6).toUpperCase()}`;
                return (<TableRow key={sale.id} className={sale.refunded ? 'opacity-50' : ''}>
                          <TableCell className="font-medium">
                            ...
                            minute: '2-digit',
                                  ...
                                  {sale.refunded && (<div className="text-xs text-destructive font-medium">Remboursé</div>)}
                                </div>)}
                            </div>
                          </TableCell>
                        ...
                        minute: '2-digit',
                          ...
                          <TableCell className="hidden lg:table-cell">
                            {sale.refunded ? (<span className="text-destructive font-medium">Remboursé</span>) : (<span className="text-green-600 font-medium">Payé</span>)}
                          </TableCell>
                          <TableCell>
                            <Button variant="outline" size="icon" onClick={() => printSaleDirect(sale)} title="Imprimer">
