...
export default function PrinterDebug() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (s) setSelected(s);
        else setSelected(localStorage.getItem('printer_mac') || null);
      } catch (e) {
  ...
  const handleConnect = async (id: string) => {
    ...
    if (res && res.ok) {
      setConnected(true);
      setSelected(id);
      try { await secureStorage.setItem('printer_mac', id); } catch (e) { console.warn('secureStorage set printer_mac failed', e); }
      try { localStorage.setItem('printer_mac', id); } catch (e) {}
        ...
        <Card className="mb-4">
          ...
          <CardContent>
            ...
            <div className="space-y-2">
              {paired.length === 0 ? (
                <div className="text-sm text-muted-foreground">Aucun appareil appairé trouvé.</div>
              ) : (
                paired.map(d => (
                  <div key={d.id} className={`p-2 border rounded flex items-center justify-between ${selected === d.id ? 'bg-muted' : ''}`}>
                    <div>
                      <div className="font-medium">{d.name}</div>
                      <div className="text-xs text-muted-foreground">{d.id}</div>
                    </div>
                    <div className="flex items-center gap-2">
                      <Button size="sm" variant={selected === d.id ? 'secondary' : 'ghost'} onClick={async () => { setSelected(d.id); try { await secureStorage.setItem('printer_mac', d.id); } catch (e) { console.warn('secureStorage set printer_mac failed', e); } try { localStorage.setItem('printer_mac', d.id); } catch (e) {} toast.success('Imprimante sélectionnée'); }}>Sélectionner</Button>
        ...
        <Card>
          ...
          <CardContent>
            <div style={{ maxHeight: 240, overflow: 'auto' }}>
              {logs.length === 0 ? <div className="text-sm text-muted-foreground">Aucun log</div> : (
