...
export default function Pin({ overlay = false }: { overlay?: boolean }) {
      ...
      try {
        ...
        if (!storedUser) {
          try {
            ...
            if (storedUser) {
              try { localStorage.setItem('pos-user', storedUser); } catch (e) {}
            }
          } catch (e) {
            console.warn('secureStorage read pos-user in Pin effect', e);
          }
        }
        if (!storedUser) {
          navigate('/login');
      ...
      try {
        ...
        if (!stored) {
          try {
            ...
            if (stored) try { localStorage.setItem('pos-user', stored); } catch (e) {}
          } catch (e) {
            console.warn('secureStorage read pos-user in Pin storedPin effect', e);
          }
        }
        if (!stored) return;
        ...
        if (mounted) setStoredPinDigits(pinRaw || null);
      } catch (e) {
        console.warn('Erreur lecture PIN local', e);
      }
    })();
    ...
    try {
      if (storedPinDigits && storedPinDigits.length > pin.length) {
        ...
        if (!Number.isFinite(expected) || !Number.isFinite(got)) {
          ...
          if (diff === 0) setBearEmotion('happy');
          else if (diff === 1) setBearEmotion('close');
          else if (diff <= 3) setBearEmotion('sad');
          else setBearEmotion('scared');
        }
      } else {
  ...
  const submitPin = async (value: string) => {
    ...
    if (fails >= 5) {
      ...
      try {
        ...
        console.warn('logout after pin lock failed', e);
      }
      navigate('/login');
      return;
    }
  ...
  const handleLoginFallback = () => {
    ...
    try { resetPinFailures(); } catch (e) { /* noop */ }
    navigate('/login');
  };
  ...
  // restore aria-hidden and pointer-events/inert
      if (appRoot) {
        ...
        else if (prevAria !== undefined) appRoot.setAttribute('aria-hidden', prevAria);
        try {
          if (prevPointer === undefined) appRoot.style.removeProperty('pointer-events');
          else appRoot.style.pointerEvents = prevPointer;
        } catch (e) {}
        try {
          if (prevInert === undefined) (appRoot as any).inert = false;
          else (appRoot as any).inert = prevInert;
        } catch (e) {}
      }
      try {
  ...
  const overlayMarkup = (
    <div ref={containerRef} tabIndex={-1} className={`${containerClass} pin-overlay-root`} aria-modal={overlay ? 'true' : undefined} role={overlay ? 'dialog' : undefined} style={{ zIndex: 2147483647 }}>
  ...
  <div className={contentClass}>
        ...
        <div className="flex items-center justify-center space-x-4 mb-6">
          {[0,1,2,3].map((i) => (
            <div key={i} className={`w-4 h-4 rounded-full border-2 ${i < pin.length ? 'bg-blue-600 border-blue-600' : 'border-gray-300'}`} />
          ))}
        </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {['1','2','3','4','5','6','7','8','9'].map((k) => (
            <button
              key={k}
              onClick={(e) => { pushDigit(k); (e.currentTarget as HTMLButtonElement).blur(); }}
              className="h-20 bg-white border rounded-lg flex items-center justify-center text-2xl shadow-sm transform transition-transform duration-150 active:scale-95 hover:shadow-md focus:scale-95 focus:outline-none focus:ring-2 focus:ring-blue-400"
          ...
          <button
            onClick={(e) => { pushDigit('0'); (e.currentTarget as HTMLButtonElement).blur(); }}
            className="h-20 bg-white border rounded-lg flex items-center justify-center text-2xl shadow-sm transform transition-transform duration-150 active:scale-95 hover:shadow-md focus:scale-95 focus:outline-none focus:ring-2 focus:ring-blue-400"
          ...
          <button
            onClick={(e) => { handleDelete(); (e.currentTarget as HTMLButtonElement).blur(); }}
            className="h-20 bg-white border rounded-lg flex items-center justify-center text-base shadow-sm transform transition-transform duration-150 active:scale-95 hover:shadow-md focus:scale-95 focus:outline-none focus:ring-2 focus:ring-red-400"
            aria-label="Effacer"
          >
            Effacer
          </button>
        ...
        <div className="mt-6 text-center">
          {locked ? (
            ...
            showAttempts && (
              <p className="text-sm text-muted-foreground">Tentatives restantes: {attemptsLeft()}</p>
            )
          )}
        </div>
      </div>
