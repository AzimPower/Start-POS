...
import {
  CreditCard, RefreshCw, Plus, Trash2, Search, Store,
  TrendingUp, Calendar, Wallet, ArrowLeft, ChevronLeft,
  ChevronRight, Crown, Filter, FileDown, AlertTriangle,
...
export default function SubscriptionPayments() {
  ...
  // ── Period range ──────────────────────────────────────────────────────────
  const periodRange = useMemo<{ start: number; end: number } | null>(() => {
    const now = new Date();
  ...
  const filteredPayments = useMemo(() => {
    return payments.filter((p) => {
      ...
      if (periodRange) {
        if (p.paidAt < periodRange.start || p.paidAt > periodRange.end) return false;
      }
      if (search.trim()) {
  ...
  const totalEncaisse = useMemo(
    () => filteredPayments.reduce((a, p) => a + p.amount, 0),
    [filteredPayments]
  );
  ...
  const avgPerPayment =
    filteredPayments.length > 0 ? totalEncaisse / filteredPayments.length : 0;
  ...
  const perStoreTotals = useMemo(() => {
    ...
    const source = periodRange
      ...
      : payments;
    source.forEach((p) => {
      if (!map[p.storeId]) map[p.storeId] = { name: p.storeName, total: 0, count: 0 };
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-7xl mx-auto px-4 -mt-4 pb-10 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <KPICard
            title={periodFilter === 'all' ? 'Total encaissé' : 'Encaissé (période)'}
            value={formatCFA(totalEncaisse)}
          ...
          <KPICard
            title="Ce mois-ci"
            value={formatCFA(thisMonthTotal)}
            subtitle={`${thisMonthPayments.length} paiement${thisMonthPayments.length !== 1 ? 's' : ''}`}
            icon={Calendar}
            color="bg-indigo-500"
          />
          <KPICard
            title="Boutiques payantes"
            value={uniqueStoresPaying}
            subtitle={`sur ${stores.length} boutiques`}
            icon={Store}
            color="bg-purple-500"
          />
          <KPICard
            title="Montant moyen"
            value={formatCFA(avgPerPayment)}
            subtitle="par encaissement"
            icon={DollarSign}
            color="bg-sky-500"
          />
        </div>
            ...
            <div className="flex flex-wrap gap-3 items-end">
              ...
              <div className="flex-1 min-w-48">
                ...
                <div className="relative">
                  ...
                  <Input
                    placeholder="Boutique, note, montant…"
                    value={search}
              ...
              {/* Reset filters */}
              {(storeFilter !== 'all' || periodFilter !== 'all' || search) && (
                <Button
                  variant="outline"
                  size="sm"
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={220}>
                <AreaChart data={chartData} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
                  ...
                  />
                  <Tooltip
                    formatter={(v: number) => [formatCFA(v), 'Encaissements']}
                ...
                <ResponsiveContainer width="100%" height={220}>
                  ...
                  >
                    ...
                    <Bar dataKey="total" radius={[0, 4, 4, 0]}>
                      {perStoreTotals.slice(0, 6).map((_, i) => (
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2">
                {perStoreTotals.map((s, i) => (
                  <button
                    key={s.id}
                    onClick={() => setStoreFilter(storeFilter === s.id ? 'all' : s.id)}
          ...
          <div className="space-y-4 py-2">
            <div>
              ...
              <Select
                ...
                onValueChange={(v) => setAddForm((f) => ({ ...f, storeId: v }))}
              >
                <SelectTrigger>
            ...
            <div>
              ...
              <Input
                placeholder="Ex: Paiement Cash, Mobile Money..."
                value={addForm.note}
