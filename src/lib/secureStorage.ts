    ...
    if (cap && cap.Plugins) {
      ...
      if (p && typeof p.get === 'function') {
        try {
          ...
          console.warn('secureStorage plugin get error', e);
        }
      }
    }
      ...
      if (SecureCtor) {
        return await new Promise<string | null>((resolve) => {
          try {
            ...
            const onInit = () => {
              try {
                instance.get((val: any) => resolve(String(val)), (err: any) => { console.warn('cordova secure get err', err); resolve(null); }, key);
              } catch (e) {
                console.warn('cordova secure get exception', e);
                resolve(null);
              }
            };
            const onError = (err: any) => { console.warn('cordova secure init err', err); resolve(null); };
            // Some plugin implementations expect new ctor(success, error, namespace)
            try {
              instance = new SecureCtor(onInit, onError, 'pos-app');
            } catch (e) {
              // If construction fails, resolve null
              console.warn('cordova secure ctor failed', e);
              resolve(null);
            }
          } catch (e) {
            console.warn('cordova secure get outer exception', e);
            resolve(null);
          }
        });
      }
...
export async function setItem(key: string, value: string): Promise<void> {
  try {
    ...
    if (cap && cap.Plugins) {
      ...
      if (p && typeof p.set === 'function') {
        try {
          ...
          console.warn('secureStorage plugin set error', e);
        }
      }
    }
    ...
    try {
      ...
      console.warn('secureStorage fallback set error', e);
    }
  } catch (err) {
    // ignore
  }
...
export async function removeItem(key: string): Promise<void> {
  try {
    ...
    if (cap && cap.Plugins) {
      ...
      if (p && typeof p.remove === 'function') {
        try {
          ...
          console.warn('secureStorage plugin remove error', e);
        }
      }
    }
    ...
    try {
      ...
      console.warn('secureStorage fallback remove error', e);
    }
  } catch (err) {
    // ignore
  }
