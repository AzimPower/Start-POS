...
export function textToEscPos(text: string) {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (code <= 0x7f) {
      ...
      const latin1Map: { [code: number]: number } = {
        0x00E9: 0xE9, // é
        0x00E8: 0xE8, // è
        0x00EA: 0xEA, // ê
        0x00EB: 0xEB, // ë
...
export async function isNativePrinterAvailable(): Promise<boolean> {
  try {
    ...
    console.warn('isNativePrinterAvailable detection error', err);
  }
  return false;
}
...
export async function listPaired(): Promise<Array<{ name: string; id: string }>> {
  try {
    const win = window as any;
    ...
    console.warn('listPaired error', err);
  }
  return [];
}

export async function connect(deviceId: string): Promise<{ ok: boolean; error?: string }> {
  try {
    const win = window as any;
    // If already connected to this device, resolve
    if (_connectedMac === deviceId) return { ok: true };
    if (win.bluetoothSerial && typeof win.bluetoothSerial.connect === 'function') {
      ...
      const attempt = (fnName: 'connect' | 'connectInsecure', id: string) => new Promise<{ ok: boolean; error?: string }>((resolve) => {
        try {
          const fn = (win.bluetoothSerial as any)[fnName];
          if (typeof fn !== 'function') return resolve({ ok: false, error: 'no_method' });
          fn.call(win.bluetoothSerial, id, () => {
            _connectedMac = id;
            resolve({ ok: true });
          }, (err: any) => {
            const em = err && (err.message || err) ? String(err.message || err) : 'error';
            console.warn(`bluetoothSerial.${fnName} err`, err);
            resolve({ ok: false, error: em });
          });
        } catch (e) {
          console.warn(`bluetoothSerial.${fnName} exception`, e);
      ...
      const attempt = (fnName: 'connect' | 'connectInsecure', id: string) => new Promise<{ ok: boolean; error?: string }>((resolve) => {
        try {
          ...
          fn.call(plugin, id, () => {
            ...
            const em = err && (err.message || err) ? String(err.message || err) : 'error';
            console.warn(`cordova.${fnName} err`, err);
            resolve({ ok: false, error: em });
          });
        } catch (e) {
          console.warn(`cordova.${fnName} exception`, e);
          resolve({ ok: false, error: String(e) });
        }
      });
...
export async function disconnect(): Promise<boolean> {
  try {
      ...
      return await new Promise((resolve) => {
        try {
          win.bluetoothSerial.disconnect(() => {
            _connectedMac = null;
            resolve(true);
          }, (err: any) => { console.warn('bt disconnect err', err); _connectedMac = null; resolve(false); });
        } catch (e) {
          console.warn('bt disconnect exception', e);
          _connectedMac = null;
          resolve(false);
        }
      });
    ...
    if (win.cordova && win.cordova.plugins && win.cordova.plugins.BluetoothSerial && typeof win.cordova.plugins.BluetoothSerial.disconnect === 'function') {
      return await new Promise((resolve) => {
        try {
          win.cordova.plugins.BluetoothSerial.disconnect(() => { _connectedMac = null; resolve(true); }, (err: any) => { console.warn('cordova disconnect err', err); _connectedMac = null; resolve(false); });
        } catch (e) {
          console.warn('cordova disconnect exception', e);
          _connectedMac = null;
          resolve(false);
        }
      });
  ...
  return new Promise<boolean>(async (resolve) => {
    try {
      ...
      if (!_connectedMac && deviceId) {
        ...
        if (!okc) {
          console.warn('Auto-connect failed for', deviceId);
        }
      }
      ...
      const tryWriteWithPlugin = (pluginWrite: any, data: any) => new Promise<boolean>((res) => {
        try {
          pluginWrite(data, () => res(true), (err: any) => { console.warn('bt write err', err); res(false); });
        } catch (e) {
      ...
      console.warn('sendToBluetoothSerial error', err);
      resolve(false);
    }
  });
}
...
export async function nativePrint(html: string, fileName?: string): Promise<boolean> {
  ...
  payload.push(...textToEscPos(text));
    ...
    console.warn('nativePrint error', err);
  }
  return false;
}
...
export async function printHtml(html: string, deviceId?: string): Promise<boolean> {
  try {
    ...
    if (deviceId) {
      ...
      if (!res || !res.ok) {
        console.warn('printHtml: connect failed', res);
        return false;
      }
    }
    return await nativePrint(html, undefined);
  } catch (err) {
    console.warn('printHtml error', err);
    return false;
  }
}
...
export async function printText(lines: string[], deviceId?: string): Promise<boolean> {
  ...
  payload.push(...SELECT_CP1252);
    ...
    console.warn('printText error', err);
    return false;
  }
}
...
export async function printImage(dataUrl: string, deviceId?: string, paper: '58' | '80' = '80'): Promise<boolean> {
  try {
    ...
    console.warn('printImage error', err);
    return false;
  }
}
 ...
 */
export async function probeDevice(deviceId: string, timeout = 3000): Promise<{ available: boolean; error?: string }> {
  try {
    // If already connected to this device, report available
    if (_connectedMac === deviceId) return { available: true };
