...
class EmailService {
  ...
  async sendEmail(payload: EmailPayload, retryCount = 3): Promise<EmailResponse> {
    ...
    console.log('🔍 Environnement détecté:', { isAndroid, isCapacitor, userAgent: navigator.userAgent });
    
    // Sur Android/Capacitor, utiliser directement XMLHttpRequest
    if (isAndroid || isCapacitor) {
      console.log('📱 Utilisation directe XMLHttpRequest pour Android/Capacitor');
    ...
    if (shouldUseFallback) {
      console.warn('🔄 Tentative avec méthode fallback (XMLHttpRequest) - Erreur SW/Channel détectée');
      return await this.sendEmailFallback(payload);
    }
  ...
  private async sendEmailNormal(payload: EmailPayload, retryCount = 3): Promise<EmailResponse> {
    for (let attempt = 1; attempt <= retryCount; attempt++) {
      try {
        console.log(`📧 Tentative d'envoi email ${attempt}/${retryCount}`, {
          to: payload.email,
          from: payload.name,
        ...
        if (data.ok) {
          ...
          console.warn(`❌ Erreur serveur email (tentative ${attempt}):`, data.error);
          if (attempt === retryCount) {
            return data; // Dernière tentative, renvoyer l'erreur
          }
        }
      } catch (error) {
        console.error(`❌ Erreur réseau email (tentative ${attempt}):`, {
  ...
  private async sendEmailFallback(payload: EmailPayload): Promise<EmailResponse> {
    return new Promise((resolve) => {
      console.log('🔄 Utilisation de XMLHttpRequest fallback');
      ...
      xhr.onload = function() {
        try {
          if (xhr.status === 200) {
            ...
            console.log('✅ Email envoyé via fallback', response);
            resolve(response);
          } else {
            console.error('❌ Erreur HTTP fallback:', xhr.status, xhr.statusText);
            resolve({
              ok: false,
          ...
          console.error('❌ Erreur parsing response fallback:', e);
          resolve({
            ok: false,
      ...
      xhr.onerror = function() {
        console.error('❌ Erreur réseau fallback');
        resolve({
          ok: false,
      ...
      xhr.ontimeout = function() {
        console.error('❌ Timeout fallback');
        resolve({
          ok: false,
  ...
  async canSendEmail(): Promise<boolean> {
    ...
    try {
      ...
      console.log(canSend ? '🌐 Serveur accessible pour emails' : '📴 Serveur inaccessible');
      return canSend;
    } catch (error) {
      console.warn('📴 Serveur inaccessible pour emails:', error);
      return false;
    }
  }
...
if (typeof window !== 'undefined') {
  window.addEventListener('unhandledrejection', (event) => {
    if (event.reason?.message?.includes('message channel closed') || 
      ...
      console.warn('🔄 Erreur Service Worker détectée et ignorée:', event.reason.message);
